...
abstract class AResource {
   ...
   * tab characters.
   * 
   * @param uri uri info with query parameters
   * @param jaxrx JAX-RX implementation
   ...
   * 
   * @param doc The XML {@link Document} containing the XQuery XML post request.
   * @param jaxrx current implementation
   * @return The parameters as {@link Map}.
   */
  protected Map<QueryParameter, String> getParameters(final Document doc,
    ...
    for(int i = 0; i < props.getLength(); i++) {
      ...
      addParameter(nnm.getNamedItem("name").getNodeValue(),
          nnm.getNamedItem("value").getNodeValue(), newParams, impl);
    }
   ...
   * @param newParams New query parameter map
   * @param impl Implementation parameters
   */
  private void addParameter(final String key, final String value,
    ...
    try {
      ...
      if(ep != QueryParameter.OUTPUT && ep != QueryParameter.VAR && old != null) return;
  ...
  private Map<QueryParameter, String> createMap() {
    ...
    for(final Map.Entry<Object, Object> set : props.entrySet()) {
      ...
      try {
        params.put(QueryParameter.valueOf(up.toUpperCase()),
  ...
  public Response postQuery(final String system, final InputStream input,
      final String resource, final HttpHeaders httpHeaders) {
  ...
  public Response getResource(final String system, final UriInfo uri,
      final String resource, final HttpHeaders headers) {
